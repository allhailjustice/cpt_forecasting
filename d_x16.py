...
def train():
    ...
    def myfunc(feature, summary, length):
        ...
        feature1 = tf.concat(
            (feature[pos1:pos1 + length1], -tf.ones((200 - length1, 112), dtype=tf.int32)), axis=0)
        feature2 = tf.concat(
            (feature[pos2:pos2 + length2], -tf.ones((200 - length2, 112), dtype=tf.int32)), axis=0)
    ...
    local_2 = FeatureNet()
    local_3 = FeatureNet()
    checkpoint = tf.train.Checkpoint(optimizer=optimizer, feature_net=feature_net, local_1=local_1, local_3=local_3, local_2=local_2)
    checkpoint.restore(checkpoint_prefix + '-21').expect_partial()
    print('start')
